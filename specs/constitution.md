...
All scoped capabilities — booking, scheduling, staff profiles, service catalogs, payments, memberships,
subscription plans, client data, and calendar operations — are owned and managed by the
...
Staff credentials and specialties MUST be accessible from their profile. If public reviews are added in a
future separately specified feature, they MUST be accessible from the same trust surface.
...
All scoped payment processing flows through Stripe and PayPal, which are PCI-compliant. Affirm (Buy Now,
Pay Later) is integrated via Stripe. If gift cards or retail checkout are added in a future separately specified
feature, they follow the same rule — tokenized references only, never raw card data. Client payment methods
are managed exclusively within the payment gateway's vault.
...
consistency is non-negotiable across all scoped surfaces: web pages, email templates, SMS notifications,
promotional materials, and booking confirmations. If gift cards are added later, they must follow the same
brand system. A dedicated `specs/design.md`
...
The following features are in scope for Brazilian Haven Beauty MVP v1. All are to be
implemented as first-party features for the initial launch scope:

| Domain | Features |
|---|---|
| **Booking** | 24/7 online booking, real-time slot availability, staff selection, waitlist management |
| **Promotions** | Daily deals, first-time client discounts, seasonal packages, coupon codes |
| **Staff** | Individual profiles, specialties, bio, photo gallery, seniority-based pricing tiers |
| **Notifications** | Email and SMS notifications for reminders, changes, waitlist, and promotions |
...
| **Payments** | Stripe + PayPal processing; Affirm BNPL via Stripe; saved payment methods via gateway vault |
| **Operations** | Internal calendar, service management, staff management, pricing management, customer tracking, notification management |
| **Client Profiles** | Personal account: booking history, purchase history, active memberships, saved preferences |

The following items are explicitly out of scope for MVP v1 and require separate specification:
- gift cards
...
**Version**: 1.0.1 | **Ratified**: 2026-04-09 | **Last Amended**: 2026-04-17
